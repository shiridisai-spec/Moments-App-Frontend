...
const EditMomentsPage = ({ params }) => {
  ...
  const [editMoment, { isLoading, isSuccess, error }] = useEditMomentMutation();
  ...
  const {
    ...
    isSuccess2,
    isError2,
  } = useGetSingleMomentQuery(params.momentId);
  ...
  const handleSubmit = async (e) => {
    ...
    // Log all values before appending to FormData
    ...
    if (!params.momentId) {
      console.error("momentId is undefined!");
      return; // Early return if momentId is not set
    }

    if (validate()) {
      const formData = new FormData();
      console.log("params.momentId", params.momentId);
      //   formData.append("id", params.momentId.toString());
      ...
      try {
        const editMomentResponse = await editMoment({
          id: params.momentId,
          formData,
        }).unwrap();
  ...
  useEffect(() => {
    ...
    setDescription(moment?.moment?.description);
    setCategory(moment?.moment?.category);
    if (moment?.moment?.ispublic) {
      setIsPublic(true);
    } else {
      setIsPublic(false);
    }
    setTags((prevTags) => [...prevTags, ...(moment?.moment?.tags || [])]);
    setPhoto(moment?.moment?.photo);
    setPhotoPreview(moment?.moment?.photo);
  }, [moment]);
